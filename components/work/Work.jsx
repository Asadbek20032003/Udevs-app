import Image from "next/image";

const Work = () => {
  return (
    <section className="px-[15px] py-[80px] ">
      <div className=" container m-auto ">
        <div>
          <h1 className="mm:text-[64px] mm:text-start text-[32px] text-center leading-[64px] font-extrabold text-[#1b5bf7] mb-[24px] mm:mb-[42px] ">
            How we work!
          </h1>
          <div className=" w-full max-w-[1371px] flex md:rotate-0 md:flex-row flex-col justify-around  items-start pt-[30px]  relative mt-[80px] mx-auto mb-0 bg-no-repeat bg-center bg-[position:center_80px]] bg-none md:bg-[url('data:image/svg+xml;base64,')]  ">
            <Image
              src="data:image/svg+xml;base64,"
              alt="dod"
              className="md:hidden block rotate-90 bg-no-repeat w-[150%] bg-start -left-[37%] absloute -z-1 "
            />
            <div className="my-[15px] md:my-0 px-[20px] md:px-0 md:items-start flex justify-start flex-row items-center md:flex-col   text-center">
              <Image
                className="md:w-[81px] md:h-[81px] mr-[27px] md:mr-0  w-[56px] h-[56px]"
                src="data:image/svg+xml;base64,"
                alt="contact"
              />
              <div className="flex flex-col items-start md:text-center text-justify">
                <h3 className="text-[18px]  leading-[28px] font-bold m-0 ">Сontact</h3>
                <p className="md:text-[16px] text-[14px]  md:leading-[26px] leading-[24px]  font-normal m-0 p-0 md:px-[20px] flex justify-center ">
                  Send us your project request or project idea.
                </p>
              </div>
            </div>
            <div className="my-[15px] md:my-0 px-[20px] md:px-0 md:items-start flex justify-start flex-row items-center md:flex-col   text-center  ">
              <Image
                className="md:w-[81px] md:h-[81px] mr-[27px] md:mr-0 w-[56px] h-[56px]"
                src="data:image/svg+xml;base64,"
                alt="analysis"
              />
              <div className="flex flex-col items-start text-justify">
                <h3 className="text-[18px]  leading-[28px] font-bold m-0 ">Analysis</h3>
                <p className="md:text-[16px] text-[14px]  md:leading-[26px] leading-[24px]  font-normal m-0 p-0 md:px-[20px] flex justify-center ">
                  We will contact you to clarify your project requirements.
                </p>
              </div>
            </div>
            <div className="my-[15px] md:my-0 px-[20px] md:px-0 md:items-start flex justify-start flex-row items-center md:flex-col   text-center  ">
              <Image
                className="md:w-[81px] md:h-[81px] mr-[27px] md:mr-0 w-[56px] h-[56px]"
                src="data:image/svg+xml;base64,"
                alt="offer"
              />
              <div className="flex flex-col items-start text-justify">
                <h3 className="text-[18px]  leading-[28px] font-bold m-0 ">Offer</h3>
                <p className="md:text-[16px] text-[14px]  md:leading-[26px] leading-[24px]  font-normal m-0 p-0 md:px-[20px] flex justify-center ">
                  We will provide you with our free, non-binding application.
                </p>
              </div>
            </div>
            <div className="my-[15px] md:my-0 px-[20px] md:px-0 md:items-start flex justify-start flex-row items-center md:flex-col   text-center  ">
              <Image
                className="md:w-[81px] md:h-[81px] mr-[27px] md:mr-0 w-[56px] h-[56px]"
                src="data:image/svg+xml;base64,"
                alt="team"
              />
              <div className="flex flex-col items-start text-justify">
                <h3 className="text-[18px]  leading-[28px] font-bold m-0 ">Team</h3>
                <p className="md:text-[16px] text-[14px]  md:leading-[26px] leading-[24px]  font-normal m-0 p-0 md:px-[20px] flex justify-center ">
                  SWe provide a team for your requirements.
                </p>
              </div>
            </div>
            <div className="my-[15px] md:my-0 px-[20px] md:px-0 md:items-start flex justify-start flex-row items-center md:flex-col   text-center  ">
              <Image
                className="md:w-[81px] md:h-[81px] mr-[27px] md:mr-0 w-[56px] h-[56px]"
                src="data:image/svg+xml;base64,"
                alt="start"
              />
              <div className="flex flex-col items-start text-justify">
                <h3 className="text-[18px]  leading-[28px] font-bold m-0 ">Start</h3>
                <p className="md:text-[16px] text-[14px]  md:leading-[26px] leading-[24px]  font-normal m-0 p-0 md:px-[20px] flex justify-center ">
                  You will get to know the team and we`ll get started.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Work;
